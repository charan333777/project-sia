...
export function QrViewer({ profile, url }: { profile: Profile; url: string }) {
  ...
  return (
    <div className={`qr-card qr-theme-${theme} qr-character-${character.id}`} id="sia-qr-card">
      ...
      {character.imageSrc && <div className="qr-character-medallion" aria-label={`${character.label} personality`}><img src={character.imageSrc} alt="" width="96" height="96" draggable={false} /></div>}
